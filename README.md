...
`firmware/tab5/` is the multi-session build for the **M5Stack Tab5** (ESP32-P4, 5" 1280×720 touch panel, speaker). The screen shows **one tile per live Claude Code session**, up to six, and splits itself by how many there are: one, two side by side, a 2×2 grid, a 3×2 grid. A tile is the session's status icon (the same gear, signs and overlays as the cube), its **name** above (the project's folder, `#2` and so on when several sessions share a project, or a nickname from `projects.json`), and below it a **clock** with how long the current icon has been up, reset on every change. When no session lines have arrived (a manual test, an older daemon) a single placeholder tile follows the aggregate words the cube uses.

**Tap a tile** to open its detail page: the icon with name and clock on one side, and on the other the **hook inspector** for that session: every field of the last hook event the daemon forwarded (`session_id`, `cwd`, `tool_name`, `tool_input.command`, `notification_type`, `prompt`, …) with a running age, and a short history of the previous events. Tap anywhere to go back to the tiles. A **two-tone beep** from the speaker marks a red icon appearing on a visible tile: rising for the `!` (your turn), falling for the `X` (error). The screen dims when the host stops talking and switches off after thirty minutes with every tile idle; a tap wakes it.

The whole display turns with gravity in 90° steps, like a tablet: landscape puts the icon on the left and the inspector on the right, portrait puts the icon on top and the inspector below. A turn has to go 12° past the 45° boundary and hold for half a second before the screen follows, and lying flat keeps the last orientation. Which way the BMI270 maps to the panel is compiled in from one unit (offset 3, sign +1) and can be recalibrated per unit and stored on the device: hold the Tab5 the way you use it and send `calibrate rot=N` where N is the display rotation that is upright right now (1 or 3 for landscape, 0 or 2 for portrait; if the picture is upside down, send the other landscape value). Then turn it 90°: if the picture turned the wrong way, send `calibrate sign=-1` (the current orientation stays correct). `calibrate reset` returns to the compiled defaults. With the daemon, `POST /calibrate?rot=1` and `POST /calibrate?sign=-1`.
