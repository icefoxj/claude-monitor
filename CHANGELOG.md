...
- `screenshot` and `view grid` / `view detail <id>` (protocol 6): the Tab5 dumps its display as text lines (`SCREENSHOT w= h= fmt=rgb888`, one base64 `ROW` per row, `END`) and opens a page on request, what a tap does; `tools/Get-Screenshot.ps1` turns a capture into a PNG (with setup lines, a wait, a crop and a scale) and `tools/Make-ReadmeImages.ps1` regenerates every picture of the README from the board. The AtomS3R answers `ERROR` to `screenshot` (its panel is not read back) and ignores `view`.
- Timers follow real time: the boards measure how many 33 ms frames went by between loop passes and hand that to `Ui::tick`, and the serial read only waits for what is left of the frame. The Tab5's clocks, work ring, link timeout and auto-off ran at about half speed while a big animated icon was on screen (drawing a 486 px gear takes longer than a frame); the cube's ran a little slow.
- README: a picture of every icon and overlay, the six-tile dashboard and the detail page, all captured from the Tab5 (`docs/images/`), a description of what the project does, and the installation as numbered steps.
...
- **Tab5 multi-session view**: one tile per live session (up to six), the screen split by their number (1, 2, 2×2, 3×2), each tile with the session's name above the icon and a clock below it with the time the current icon has been up (reset on every change); tap a tile for its detail page (icon, name, clock and the hook inspector for that session), tap again to return; a two-tone beep from the speaker when a red icon appears (rising for `!`, falling for `X`). Protocol 5: `session <id> label= state= subagents= tool= project=`, `session_end <id>`, `session_clear`, sent by the daemon only to boards with `sessions` in `features=`; a session line precedes the session's first event.
- Approval detection: the daemon learns each session's `claude.exe` from the hook's TCP connection and watches its child processes; a tool process starting under a session showing the red sign means the permission was approved, so the gear returns within about half a second instead of when the tool ends (measured before: median 73 s of red per approval, 90th percentile 260 s). The same events drive a new **tool running** flag (`tool_start` / `tool_stop`, protocol 4): a blue badge reading "EXT" at the bottom right of the icon while a tool process is alive; and a session whose process is gone is dropped at once.
...
- Daemon: the process watching no longer uses WMI events. It diffs snapshots of the process table (`NtQuerySystemInformation` through a small C# helper, filtered to the sessions' processes) twice a second while a permission is pending or a tool was just called, every two seconds otherwise. The WMI subscription died during an ESP-IDF build: a polling query keeps whole `Win32_Process` instances within a 5 MB per-user quota, so with a few hundred processes it was cancelled silently, new ones failed with "Quota violation", and the flood of events before that stalled the daemon for minutes with the hooks timing out. `GET /status` shows `process_watch` and `process_pids`.
