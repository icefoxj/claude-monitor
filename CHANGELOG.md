...
- **Tab5 build** (`firmware/tab5`, ESP32-P4, work in progress, not yet run on hardware): the icon on the left and a hook inspector on the right that lists every field of the last hook event with its age, plus a history of the previous ones. Touch toggles the screen. No tilt tracking.
