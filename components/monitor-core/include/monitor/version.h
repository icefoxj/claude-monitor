...
// ("atoms3r"), `features` a comma-separated list of what this build can do
// beyond the icon ("events,touch"; "" for none), which the host reads
// before sending anything optional. The line also carries the detected
// model, chip and revision, core count, flash size, app version (git
// describe at build time), ESP-IDF and M5Unified versions, protocol
// version, project name, build time (ISO 8601, local to the build
// machine), the first 8 hex digits of the ELF SHA-256, uptime in seconds
// and the reason for the last reset.
